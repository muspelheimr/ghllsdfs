...
function FauxScrollFrame_Update(frame, numItems, numToDisplay, buttonHeight, button, smallWidth, bigWidth, highlightFrame, smallHighlightWidth, bigHighlightWidth, alwaysShowScrollBar, heightOnDisplay )
	...
	if ( frame:IsShown() ) then
		...
		if ( numItems > 0 ) then
			...
			if heightOnDisplay then
				scrollChildHeight = numToDisplay * buttonHeight;
			else
...
function UIPanelButton_OnLoad(self)
	local parent = self:GetParent()
	parent.Middle:ClearAllPoints();
	parent.Middle:SetPoint("TOPLEFT", parent.Left, "TOPRIGHT");
	parent.Middle:SetPoint("BOTTOMRIGHT", parent.Right, "BOTTOMLEFT");

	if ( not parent:IsEnabled() == 1 ) then
		parent.Left:SetTexture("Interface\\Buttons\\UI-Panel-Button-Disabled");
		parent.Middle:SetTexture("Interface\\Buttons\\UI-Panel-Button-Disabled");
		parent.Right:SetTexture("Interface\\Buttons\\UI-Panel-Button-Disabled");
	end
end
