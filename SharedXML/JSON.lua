...
local strbyte, strchar, strfind, strformat, strgsub, strmatch, strsub = string.byte, string.char, string.find, string.format, string.gsub, string.match, string.sub
local strconcat = strconcat

local mininteger = -99999999999999
...
do
	...
	local function _decode_error(pos, errmsg)
		error(strformat("decode error at %d: %s", pos, errmsg), 2)
	end
	...
	local function f_nul()
		if strsub(json, pos, pos+2) == 'ull' then
			pos = pos+3
			return nullv
		end
		decode_error('invalid value')
	...
	local function f_fls()
		if strsub(json, pos, pos+3) == 'alse' then
			pos = pos+4
			return false
		end
		decode_error('invalid value')
	...
	local function f_tru()
		if strsub(json, pos, pos+2) == 'rue' then
			pos = pos+3
			return true
		end
		decode_error('invalid value')
	...
	local radixmark = strmatch(tostring(0.5), '[^0-9]')
	local fixedtonumber = tonumber
	if radixmark ~= '.' then
		if strfind(radixmark, '%W') then
			radixmark = strconcat('%', radixmark)
		end
		fixedtonumber = function(s)
			return tonumber(strgsub(s, '.', radixmark))
		end
	end
	...
	local function f_zro(mns)
		local num, c = strmatch(json, '^(%.?[0-9]*)([-+.A-Za-z]?)', pos)  -- skipping 0
		...
		if num == '' then
			...
			if c == 'e' or c == 'E' then
				num, c = strmatch(json, '^([^eE]*[eE][-+]?[0-9]+)([-+.A-Za-z]?)', pos)
				if c == '' then
					pos = pos + #num
					if mns then
						return -0.0
		...
		if strbyte(num) ~= 0x2E or strbyte(num, -1) == 0x2E then
			number_error()
		end
	...
	local function f_num(mns)
		pos = pos-1
		local num, c = strmatch(json, '^([0-9]+%.?[0-9]*)([-+.A-Za-z]?)', pos)
		if strbyte(num, -1) == 0x2E then  -- error if ended with period
			number_error()
		end
		...
		if mns then
			c = -c
			if c == mininteger and not strfind(num, '[^0-9]') then
				c = mininteger
			end
		end
		return c
	...
	local function f_str_subst(ch, ucode)
		if ch == 'u' then
			local c1, c2, c3, c4, rest = strbyte(ucode, 1, 5)
			...
			if ucode ~= inf then
				if ucode < 0x80 then  -- 1byte
					if rest then
						return strchar(ucode, rest)
					end
					return strchar(ucode)
				elseif ucode < 0x800 then  -- 2bytes
					...
					c2 = c2 + 0x80
					if rest then
						return strchar(c1, c2, rest)
					end
					return strchar(c1, c2)
				elseif ucode < 0xD800 or 0xE000 <= ucode then  -- 3bytes
					...
					c3 = c3 + 0x80
					if rest then
						return strchar(c1, c2, c3, rest)
					end
					return strchar(c1, c2, c3)
				elseif 0xD800 <= ucode and ucode < 0xDC00 then  -- surrogate pair 1st
					...
					if f_str_surrogate_prev ~= 0 then
						...
						c4 = c4 + 0x80
						if rest then
							return strchar(c1, c2, c3, c4, rest)
						end
						return strchar(c1, c2, c3, c4)
					end
		...
		return strconcat(f_str_escapetbl[ch], ucode)
	end
	...
	local function f_str(iskey)
		...
		local tmppos, c1, c2
		repeat
			newpos = strfind(json, '"', newpos, true)  -- search '"'
			if not newpos then
				decode_error("unterminated string")
			end
			tmppos = newpos-1
			newpos = newpos+1
			c1, c2 = strbyte(json, tmppos-1, tmppos)
		...
		local str = strsub(json, pos, tmppos)
		pos = newpos
		...
		if strfind(str, f_str_ctrl_pat) then
			decode_error("unescaped control string")
		end
		if strfind(str, '\\', 1, true) then  -- check whether a backslash exists
	...
	local function f_ary()
		...
		local i = 0
		if strbyte(json, pos) == 0x5D then  -- check closing bracket ']' which means the array empty
			pos = pos+1
		else
			local newpos = pos
			repeat
				i = i+1
				f = dispatcher[strbyte(json,newpos)]  -- parse value
				pos = newpos+1
				ary[i] = f()
				newpos = strmatch(json, '^[ \n\r\t]*,[ \n\r\t]*()', pos)  -- check comma
			until not newpos
	...
	local function f_obj()
		...
		pos = strmatch(json, '^[ \n\r\t]*()', pos)
		if strbyte(json, pos) == 0x7D then  -- check closing bracket '}' which means the object empty
			pos = pos+1
		else
			local newpos = pos

			repeat
				if strbyte(json, newpos) ~= 0x22 then  -- check '"'
				...
				f = f_err
				local c1, c2, c3 = strbyte(json, pos, pos+3)
				if c1 == 0x3A then
					if c2 ~= 0x20 then
						f = dispatcher[c2]
						newpos = pos+2
				...
				if f == f_err then  -- read a colon and arbitrary number of spaces
					...
					f = dispatcher[strbyte(json, newpos)]
					newpos = newpos+1
				end
				pos = newpos
	...
	decode = function(json_, pos_, nullv_, arraylen_)
		json, pos, nullv, arraylen = json_, pos_, nullv_, arraylen_
		rec_depth = 0

		pos = strmatch(json, '^[ \n\r\t]*()', pos)

		f = dispatcher[strbyte(json, pos)]
		pos = pos+1
		local v = f()
