...
local function securecallrange(array, func, ...)
	for owner, handler in SecureNext, array do
		securecall(func, owner, handler, ...)
	end
end
...
function CallbackRegistryMixin:HasRegistrantsForEvent(event)
	for callbackType, callbackTable in pairs(self:GetCallbackTables()) do
		...
		if callbacks and securecall(next, callbacks) then
			return true;
		end
	end
	return false;
...
function CallbackRegistryMixin:TriggerEvent(event, ...)
	...
	if closures then
		...
		securecallrange(closures, CallbackRegistryExecuteClosurePair, ...);
	end
	...
	if funcs then
		local function CallbackRegistryExecuteOwnerPair(owner, func, ...)
			securecall(func, owner, ...);
		end

		securecallrange(funcs, CallbackRegistryExecuteOwnerPair, ...);
	end
end
