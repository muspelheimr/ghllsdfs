...
function FCF_FadeInChatFrame(chatFrame)
	...
	for index, value in pairs(CHAT_FRAME_TEXTURES) do
		...
		if ( object:IsShown() ) then
			UIFrameFadeIn(object, CHAT_FRAME_FADE_TIME, object:GetAlpha(), max(chatFrame.oldAlpha or 0, DEFAULT_CHATFRAME_ALPHA));
		end
	end
...
function FCF_FadeOutChatFrame(chatFrame)
	...
	for index, value in pairs(CHAT_FRAME_TEXTURES) do
		...
		local object = _G[frameName..value];
		if ( object:IsShown() ) then
			UIFrameFadeOut(object, CHAT_FRAME_FADE_OUT_TIME, max(object:GetAlpha(), chatFrame.oldAlpha or DEFAULT_CHATFRAME_ALPHA), chatFrame.oldAlpha or DEFAULT_CHATFRAME_ALPHA);
		end
	end
	if ( chatFrame == FCFDock_GetSelectedWindow(GENERAL_CHAT_DOCK) ) then
		for _, frame in pairs(FCFDock_GetChatFrames(GENERAL_CHAT_DOCK)) do
