...
local InitializeSecureRaidMenu = function(self)
	local unit = self.unit;
	if( not unit ) then return end
...
SECURE_ACTIONS.toggleraidmenu = function(self, unit, button)
	-- Load the dropdown
	if( not secureDropdown ) then
		secureDropdown = CreateFrame("Frame", "SecureTemplatesDropdown", nil, "UIDropDownMenuTemplate");
		secureDropdown:SetID(1);

		UIDropDownMenu_Initialize(secureDropdown, InitializeSecureRaidMenu, "MENU");
	end
    ...
    if ( actionType ) then
       local atRisk = false;
        ...
        if ( not handler ) then
            ...
            -- GMA call allows generic click handler snippets; it's second to prevent values set on the frame from suppressing it
