...
function PetStable_OnLoad(self)
	...
	UIDropDownMenu_SetInitializeFunction(self.DropDown, function(this)
		local info = UIDropDownMenu_CreateInfo()
		info.text = PET_ABANDON
		info.notCheckable = true
		info.func = function(button)
			local icon, name, level, family, talent = GetStablePetInfo(this.slotIndex)
			local petName = string.format(ABANDON_STABLE_PET_INFO, name, level, family, talent)
			StaticPopup_Show("ABANDON_STABLE_PET", petName, nil, this.slotIndex)
		end
...
function PetStable_OnEvent(self, event, ...)
	...
	if ( event == "PET_STABLE_SHOW" ) then
		...
		PetStable_Update();
		StaticPopup_Hide("ABANDON_STABLE_PET")
...
function PetStable_ShowMenu(self)
	if self:GetID() ~= 0 and IsGMAccount() and GetStablePetInfo(self:GetID()) then
		local dropdown = self:GetParent().DropDown
		dropdown.slotIndex = self:GetID()
		ToggleDropDownMenu(1, nil, dropdown, self, 37, 0);
	end
end