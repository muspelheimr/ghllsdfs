...
function UIMenu_UpdateSize(self)
	local name = self:GetName();

	local numButtons, numHiddenButtons = 0, 0;
	for id = 1, self.numButtons do
		local button = _G[name.."Button"..id];

		if button.shownFunc and not button.shownFunc() then
			button:Hide();

			numHiddenButtons = numHiddenButtons + 1;
		else
			numButtons = numButtons + 1;

			button:ClearAllPoints();
			button:SetPoint("TOP", self, "TOP", 0, -((numButtons - 1) * UIMENU_BUTTON_HEIGHT) - UIMENU_BORDER_HEIGHT);
		end
	end

	self:SetHeight(((self.numButtons - numHiddenButtons) * UIMENU_BUTTON_HEIGHT) + (UIMENU_BORDER_HEIGHT * 2));
end

function UIMenu_OnShow(self)
	UIMenu_UpdateSize(self);

	self.timeleft = UIMENU_TIMEOUT;
	self.counting = 0;
end

function UIMenu_AddButton(self, text, shortcut, func, nested, value, shownFunc)
