...
local SECURE_IsForbidden = SECURE_IsForbidden
local SECURE_SetForbidden = SECURE_SetForbidden
...
function HANDLE:GetRect()
	return LOCAL_CHECK_Frame.GetRect(GetHandleFrame(self));
end
...
--	if LOCAL_CHECK_Frame.IsForbidden(frame) then
	if SECURE_IsForbidden(frame) then
		return false;
	end
...
function HANDLE:GetPoint(i)
    local point, frame, relative, dx, dy = LOCAL_CHECK_Frame.GetPoint(GetHandleFrame(self), i);
    local handle;
    if (frame) then
