...
function EquipmentFlyout_Show(itemButton)
	...
	if useItemLocation then
		...
		local function ItemLocationSort(lhsLocation, rhsLocation)
			...
			local lhsItemID = locationToItemID[lhsLocation] or -1;
			local rhsItemID = locationToItemID[rhsLocation] or -1;
...
function EquipmentFlyout_DisplayButton(button, paperDollItemSlot)
	...
	local itemID, name, textureName, count, durability, maxDurability, invType, locked, start, duration, enable, setTooltip = EquipmentManager_GetItemInfoByLocation(location);
